...
hook.Add( "HUDPaint", "HuntHud", function()
	if LocalPlayer():Alive() and LocalPlayer():IsValid() then
		...
		draw.DrawText( "Счет: "..LocalPlayer():GetNWInt("Score").."", "TargetID", ScrW() * 0.03, ScrH() * 0.82, score_color, TEXT_ALIGN_LEFT )
		draw.DrawText( LIGHTEXT, "TargetID", ScrW() * 0.03, ScrH() * 0.84, LIGHTCOLOR, TEXT_ALIGN_LEFT )
		draw.DrawText( "Свет: "..math.Round(lightcol,1).."", "TargetID", ScrW() * 0.03, ScrH() * 0.86, darkencolor, TEXT_ALIGN_LEFT )
		draw.DrawText( HUDTEXT, "TargetID", ScrW() * 0.03, ScrH() * 0.88, HUDCOLOR, TEXT_ALIGN_LEFT )
		if ShowScoreboard==true then
			draw.DrawText( "Убийств: "..LocalPlayer():GetNWInt("Kills").."", "TargetID", ScrW() * 0.03, ScrH() * 0.74, Color(0,255,0), TEXT_ALIGN_LEFT )
			draw.DrawText( "Бесшумных убийств: "..LocalPlayer():GetNWInt("SilentKills").."", "TargetID", ScrW() * 0.03, ScrH() * 0.76, Color(0,255,0), TEXT_ALIGN_LEFT )
			draw.DrawText( "Смертей: "..LocalPlayer():GetNWInt("Deaths").."", "TargetID", ScrW() * 0.03, ScrH() * 0.78, Color(0,255,0), TEXT_ALIGN_LEFT )
		end
			if (input.IsKeyDown(32)) then
				draw.DrawText( "Свет: "..math.Round(vlight,1).."", "TargetID", ScrW() * 0.5, ScrH() * 0.52, vdarkencolor, TEXT_ALIGN_CENTER )
		end
		if disguised==1 then
		draw.DrawText( "Замаскирован", "TargetID", ScrW() * 0.5, ScrH() * 0.90, Color(0,255,0), TEXT_ALIGN_CENTER )
		end
	end
end)
...
hook.Add( "PreDrawHalos", "AddHalos", function()
	if LocalPlayer():Alive() and LocalPlayer():GetPos() and disguised ==0 then
		for k, v in pairs(ents.FindInSphere(LocalPlayer():GetEyeTraceNoCursor().HitPos, GetConVarNumber(""))) do
...
if (trace.Entity:Health() > 0) then
	text = "Здоровье: "..trace.Entity:Health().. ""
	font = "TargetIDSmall"
end
