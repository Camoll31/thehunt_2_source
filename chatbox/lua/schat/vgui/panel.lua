...
function PANEL:Init()
	...
	self.TimeCheck:FormatCheck("Время сообщения")
	...
	self.SoundCheck:FormatCheck("Звуки")

	self.CloseB = vgui.Create("DButton", self)
	self.CloseB:SetPos(self:GetWide() - 100, 0)
	self.CloseB:SetText("")
	self.CloseB:SetSize(100, 24)
	...
	self.EnterB:SetPos(self:GetWide() - 105, self:GetTall() - 25)
	self.EnterB:SetSize(100, 21)
	self.EnterB:SetText("")

	self.TextEntry = vgui.Create("DTextEntry", self)
	self.TextEntry:SetSize(self.Width - 115, 21)
...
function PANEL:Paint()		
	...
	self.CloseB.Paint = function()
		...
		draw.SimpleText("Настройки", "SChatTitle", self.CloseB:GetWide()/2, self.CloseB:GetTall()/2, Color(255, 255, 255, self.Alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
	...
	self.EnterB.Paint = function()
		...
		if self.TeamBased then		
			...
			draw.SimpleText("Отправить", "SChatText", self.EnterB:GetWide()/2, self.EnterB:GetTall()/2, col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	end
